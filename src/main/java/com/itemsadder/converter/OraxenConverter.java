...
public class OraxenConverter extends AbstractConverter
{
    private final File sourceFolder;
    private final boolean toIaContents;
    private final String folderName;

    public OraxenConverter(String folderName, boolean toIaContents) throws IllegalArgumentException
    {
        this.folderName = folderName;
        this.sourceFolder = new File(Main.inst().getDataFolder(), new File("convert", folderName).getPath());
        this.toIaContents = toIaContents;
        if (!sourceFolder.exists())
        {
            throw new IllegalArgumentException("Folder " + folderName + " does not exist in the converter directory. " + sourceFolder.getPath());
        }
    }
    ...
    public void convert()
    {
        ...
        if (itemsFolder.exists())
        {
            ...
            for (File file : itemsFiles)
            {
                ...
                outputConfig.set("info.namespace", "oraxen");
                outputConfig.set("info.converted_from", "oraxen");
                ...
                // TODO other various Oraxen mechanics
                //  https://docs.oraxen.com/mechanics/all-mechanics

                // TODO: convert sounds to the json variant. Or wait ItemsAdder to implement sounds as yml.
                //  https://github.com/PluginBugs/Issues-ItemsAdder/issues/4489
                ...
                try
                {
                    ...
                    if(toIaContents)
                    {
                        outputFolder = new File(Main.inst().getDataFolder().getParent(), "ItemsAdder" + File.separator + "contents" + File.separator + "oraxen");
                    }
                    else
                    {
                        outputFolder = new File(Main.inst().getDataFolder(), "converted" + File.separator + folderName);
                    }
                    ...
                    outputConfig.save(new File(outputFolder, file.getName()));
                    Main.inst().getLogger().info("Converted file: " + file.getName() + " to " + outputFolder.getPath());
                }
                catch (IOException e)
                {
        ...
        File packFolder = new File(sourceFolder, "pack");
        if (packFolder.exists())
        {
            try
            {
                ...
                if (toIaContents)
                {
                    outputFolder = new File(Main.inst().getDataFolder().getParent(), "ItemsAdder" + File.separator + "contents" + File.separator + "oraxen" + File.separator + "resource_pack");
                }
                else
                {
                    outputFolder = new File(Main.inst().getDataFolder(), "converted" + File.separator + "oraxen" + File.separator + "resource_pack");
                }

                // Oraxen is omitting the namespace in its `pack` folder. By default, it is `minecraft`.
                List<String> validAssetsMinecraftSubFolders = List.of("models", "textures", "font", "lang", "blockstates", "shaders", "equipment", "items", "particles", "post_effect", "texts", "atlases");
                File destinationMinecraftFolder = new File(outputFolder, "assets/minecraft");
                FileUtils.forceMkdir(destinationMinecraftFolder);

                for (String assetsMinecraftSubFolderName : validAssetsMinecraftSubFolders)
                {
                    File assetsMinecraftSubFolder = new File(packFolder, assetsMinecraftSubFolderName);
                    if (assetsMinecraftSubFolder.exists() && assetsMinecraftSubFolder.isDirectory())
                    {
                        File target = new File(destinationMinecraftFolder, assetsMinecraftSubFolderName);
                        FileUtil.copyDirectoryContentsSkippingExtensions(assetsMinecraftSubFolder, target, Set.of("zip"));
                    }
                }

                // Move also the assets folder, if in the `pack` folder.
                // I think it's there to allow specifying namespaces for assets that are not meant to be in the `minecraft` default namespace.
                File sourceAssetsFolder = new File(packFolder, "assets");
                if (sourceAssetsFolder.exists() && sourceAssetsFolder.isDirectory()) {
                    File targetAssetsFolder = new File(outputFolder, "assets");
                    FileUtil.copyDirectoryContentsSkippingExtensions(sourceAssetsFolder, targetAssetsFolder, Set.of("zip"));
                }

                Main.inst().getLogger().info("Moved resource pack: " + packFolder.getName() + " to " + outputFolder.getPath());
            }
            catch (IOException e)
            {
                Main.inst().getLogger().warning("Failed to save resourcepack: " + packFolder.getName());
                e.printStackTrace();
            }
        }
    }
    ...
    protected void convertItems(File file, FileConfiguration config, FileConfiguration outputConfig)
    {
        Mapping mapping = Mapping.create()
                ...
                .custom((source, dest) -> {
                    ...
                    if(!pack.getBoolean("generate_model"))
                    {
                        resource.set("generate", false);
                        String modelPath = pack.getString("model");
                        if(modelPath != null)
                        {
                            modelPath = sanitizeNamespacedString(modelPath);
                            resource.set("model_path", modelPath);
                        }
                        else
                        {
                            warn("'Pack.model' not found, but 'Pack.generate_model' is set to false. Model path is required.", source, file);
                        }
                    }
                    else
                    {
                        ...
                        // texture (basic)
                        if (pack.contains("texture") && pack.get("texture") instanceof String texture)
                        {
                            texture = sanitizeNamespacedString(texture);
                            resource.set("texture", texture);
                        }
                        else
                        {
                            ...
                            String defaultTexture = !textures.isEmpty() ? textures.get(0) : "block/missing_texture";

                            List<String> texturesList = switch (parentModel)
                            {
                                case "block/cube", "block/cube_directional", "block/cube_mirrored" -> List.of(
                                        getTexture(textures, 2, defaultTexture), // down
                                        getTexture(textures, 4, defaultTexture), // east
                                        getTexture(textures, 2, defaultTexture), // north
                                        getTexture(textures, 3, defaultTexture), // south
                                        getTexture(textures, 1, defaultTexture), // up
                                        getTexture(textures, 5, defaultTexture)  // west
                                );
                                case "block/cube_all", "block/cube_mirrored_all" ->
                                        Collections.nCopies(6, defaultTexture);
                                case "block/cube_top" -> List.of(
                                        getTexture(textures, 1, defaultTexture), // down
                                        getTexture(textures, 1, defaultTexture), // east
                                        getTexture(textures, 1, defaultTexture), // north
                                        getTexture(textures, 1, defaultTexture), // south
                                        getTexture(textures, 0, defaultTexture), // up
                                        getTexture(textures, 1, defaultTexture)  // west
                                );
                                case "block/cube_bottom_top" -> List.of(
                                        getTexture(textures, 2, defaultTexture), // down
                                        getTexture(textures, 1, defaultTexture), // east
                                        getTexture(textures, 1, defaultTexture), // north
                                        getTexture(textures, 1, defaultTexture), // south
                                ...
                                case "block/orientable", "block/orientable_with_bottom", "block/orientable_vertical" ->
                                {
                                    String front = getTexture(textures, 0, defaultTexture);
                                    String side = getTexture(textures, 1, defaultTexture);
                                    String top = getTexture(textures, 2, defaultTexture);
                                    String bottom = getTexture(textures, 3, defaultTexture);
                                    yield List.of(
                                ...
                                case "block/cube_column" -> List.of(
                                        getTexture(textures, 0, defaultTexture), // down (end)
                                        getTexture(textures, 1, defaultTexture), // east (side)
                                        getTexture(textures, 1, defaultTexture), // north
                                        getTexture(textures, 1, defaultTexture), // south
                                        getTexture(textures, 0, defaultTexture), // up (end)
                                        getTexture(textures, 1, defaultTexture)  // west
                                );
                                case "block/cross" ->
                                {
                                    warn("'block/cross' not supported by ItemsAdder! Using single texture as fallback.", source, file);
                                    yield List.of(defaultTexture);
                                }
                                default -> textures.isEmpty() ? List.of(defaultTexture) : textures;
                            };

                            texturesList = texturesList.stream()
                                    .map(this::sanitizeNamespacedString)
                                    .toList();

                            resource.set("parent", parentModel);

                            if (texturesList.size() == 1)
                            {
                                resource.set("texture", texturesList.get(0));
                            }
                            else
                            {
                                resource.set("textures", texturesList);
                            }
                        }
                    }
                ...
                .custom((source, dest) -> {
                    ...
                    {
                        // TODO, implement automatic creation of the recipe for repairable?
                        ...
                        List.of(
                            ...
                            if (components.contains(s))
                                warn("'Components." + s + "' not supported by ItemsAdder.", source, file);
                        });
                    }

                    // Unsupported mechanics
                    if(source.contains("Mechanics"))
                    {
                        ConfigurationSection mechanics = source.getConfigurationSection("Mechanics");
                        ...
                        List.of(
                            ...
                            if (mechanics.contains(s))
                                warn("'Mechanics." + s + "' not supported by ItemsAdder.", source, file);
                        });
                    }
                })
                // tooltip_display
                ...
                // durability
                .custom((source, dest) -> {
                    // https://docs.oraxen.com/mechanics/all-mechanics#durability
                    //     durability: 10
                    ...
                    if (source.get("Components.durability", source.get("Components.durability.value")) instanceof Integer durability)
                    {
                        dest.set("durability.max_durability", durability);
                    }

                    if (source.contains("Components.durability.damage_block_break"))
                        warn("'durability.damage_block_break' support not supported.", source, file);
                    if (source.contains("Components.durability.damage_entity_hit"))
                ...
                // bigmining
                .custom((source, dest) -> {
                    if (source.contains("Mechanics.bigmining"))
                    {
                        // ItemsAdder
                        //     events:
                        //      block_break:
                        ...
                        //          size: 1
                        //          depth: 1
                        //          keep_ores: false # false by default
                        //          drop_all_blocks:
                        //            enabled: true # true by default
                        //            need_silk_touch: false # false by default
                        ...
                        dest.set("events.block_break.multiple_break.size", source.getInt("Mechanics.bigmining.radius", 1));
                        dest.set("events.block_break.multiple_break.depth", source.getInt("Mechanics.bigmining.depth", 1));
                        // Other properties are not available in Oraxen, so I set them as default
                        dest.set("events.block_break.multiple_break.drop_all_blocks.enabled", true);
                    }
                })
                // TODO: hat mechanic.
                // TODO: clickAction mechanic. Use the events of ItemsAdder.
                // TODO: armor_effects mechanic. Use the events of ItemsAdder.
                // TODO: commands mechanic. Use the events of ItemsAdder.
                // TODO: blocks mechanics, use the specific_properties of ItemsAdder.
                // TODO: music_disc mechanic.
                // TODO: food mechanic.
                ;
    ...
    /**
     * Note: when the namespace is missing from models and textures, it defaults to "minecraft" in Oraxen.
     * In contrast, ItemsAdder uses the namespace of the current file.
     * Oraxen has no information about the namespace in the current file.
     */
    private String sanitizeNamespacedString(String str)
    {
        if(!str.contains(":"))
            str = "minecraft:" + str;
        return str;
    ...
    private static String getTexture(List<String> layers, int index, String def)
    {
        return index < layers.size() ? layers.get(index) : def;
    }
    ...
    // TODO: implement live converter logic that can convert ingame items and placed furniture.
}
